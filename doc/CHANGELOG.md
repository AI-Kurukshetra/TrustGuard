...
- Added transaction triage endpoint `PATCH /api/transactions/{id}` (`app/api/transactions/[id]/route.ts`) and `updateTransactionStatus()` support in `lib/trustguard-data.ts` to allow analyst-driven decision updates (`approved`, `review`, `blocked`).
- Replaced static Transactions table with `components/transactions/transactions-manager.tsx` featuring queue filters, search, risk-based recommended actions, and one-click status controls.
- Upgraded `components/cases/case-manager.tsx` with searchable queue, explicit status selector + save, recommended-status shortcut, and clearer action feedback messaging.
- Upgraded `components/alerts/alert-manager.tsx` with search, bulk “mark visible open as reviewed,” inline success feedback, and direct handoff link to case queue for critical/high alerts.
- Updated in-product API references/docs (`lib/api-reference.ts`, `doc/API.md`) to include manual transaction status update endpoint coverage.
